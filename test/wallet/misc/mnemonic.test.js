...
describe('binToMnemonic', () => {
  it('should throw if seed byte count is not a multiple of 3 ', () => {
    expect(() => {
      binToMnemonic(Buffer.from(new Uint8Array(47)));
    }).to.throw();
  });
  it('does not throw if seed byte count is a multiple of 3 ', () => {
    expect(() => {
      binToMnemonic(Buffer.from(new Uint8Array(48)));
    }).to.not.throw();
  });
  it('produces a 32 word list from 48 bytes of passed data', () => {
    expect(binToMnemonic(Buffer.from(new Uint8Array(48))).split(' ').length).to.equal(32);
  });
  it('produces valid mnemonic from hexseed', () => {
    const mnemonicOutput = binToMnemonic(Buffer.from(HEX_SEED, 'hex'));
...
describe('mnemonicToBin', () => {
  ...
  it('should produce valid hexseed from valid input mnemonic', () => {
    const output = mnemonicToBin(MNEMONIC);
    expect(Buffer.from(output).toString('hex')).to.equal(HEX_SEED);
  });
  ...
  // TODO(rgeraldes24)
  it.skip('should throw seed output size is invalid', () => {
