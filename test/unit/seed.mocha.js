...
describe('wallet/common/seed', () => {
    describe('Seed', () => {
        ...
        it('throws on invalid sizes', () => {
            expect(() => new Seed(Uint8Array.from([1]))).to.throw(
                `Seed must be ${SEED_SIZE} bytes`,
            );
            expect(() => Seed.from('0xdead')).to.throw(
                `Seed: expected ${SEED_SIZE} bytes, got 2`,
            );
        });
    });
    ...
    describe('ExtendedSeed', () => {
        ...
        it('throws on invalid sizes', () => {
            expect(() => new ExtendedSeed(Uint8Array.from([1]))).to.throw(
                `ExtendedSeed must be ${EXTENDED_SEED_SIZE} bytes`,
            );
            expect(() => ExtendedSeed.from('0xdead')).to.throw(
                `ExtendedSeed: expected ${EXTENDED_SEED_SIZE} bytes, got 2`,
            );
        });
    });
})